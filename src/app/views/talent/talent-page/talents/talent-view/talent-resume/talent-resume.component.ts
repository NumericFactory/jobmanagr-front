...
@Component({
  ...
  styleUrls: ['./talent-resume.component.css']
})
export class TalentResumeComponent {
  ...
  uploadResumeAction(event: Event, cvinput: HTMLInputElement) {
  ...
  downloadResumeAction(talent: TalentModel, resumeId: number) {
    this.talentGateway.downloadTalentResume(talent, resumeId);
  }

  viewResumeAction(talent: TalentModel, resumeId: number) {
    this.talentGateway.viewTalentResume(talent, resumeId);
  }

  deleteResumeAction(talent: TalentModel, resumeId: number) {
    ...
    this.deleteConfirmationDialogRef.afterClosed().subscribe((isDeleteConfirmation: boolean) => {
      if (isDeleteConfirmation) {
        this.talentGateway.deleteTalentResume(this.talent, resumeId);
      }
    });
  }
